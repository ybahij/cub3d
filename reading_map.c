...
/*   Updated: 2025/01/02 09:16:56 by ansoulai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (parse_nodes_directions(head, player) || parse_nodes_colors(head, player))
		exit_w_message("Error\n Invalid directions or Colors Structure \n");
	return (0);
}
...
void	reading_map(char *av, t_player *player)
{
	...
	fd = open_map_file(av);
	while ((line = get_next_line(fd)) != NULL && (!ft_isdigit(line[0])
			&& line[0] != ' ' && line[0] != '1'))
	{
		if (line[0] != '\n' && line[0] != '\0')
		{
